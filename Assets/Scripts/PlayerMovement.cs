...
public class PlayerMovement : MonoBehaviour
{
    ...
    [Header("Jumping")] [SerializeField] float gravity = -9.81f; // test
    ...
    [Header("Crouching")] [SerializeField] float crouchSpeed = 2;
    ...
    [SerializeField]
    float rayHeightOffset;

    [SerializeField] float climbSpeed;
    int layerID = 8;
    LayerMask scalableWallsMask;

    [Header("SFX")] [SerializeField] AudioSource playerAudio;
    ...
    void Crouching()
    {
        ...
        if (Input.GetKey(KeyCode.C))
        {
            ...
            if (gameObject.GetComponent<CapsuleCollider>().center.y >
                centerChange) // change the center of the collider to match crouch hitbox
            {
                gameObject.GetComponent<CapsuleCollider>().center = new Vector3(capsuleColliderCenter.x,
                    capsuleColliderCenter.y + centerChange, capsuleColliderCenter.z);
            }
        }
        else
        {
            gameObject.GetComponent<CapsuleCollider>().height = crouchHeight * 2;
            gameObject.GetComponent<CapsuleCollider>().center =
                new Vector3(capsuleColliderCenter.x, 0f, capsuleColliderCenter.z);
            speed = 5;
        }
    }
    ...
    void OnWall()
    {
        ...
        if (Input.GetKey(KeyCode.W))
        {
            ...
            AnimationRegistry.PlayAnimation("climb_up");
        ...
        if (Input.GetKey(KeyCode.S))
        {
            ...
            AnimationRegistry.PlayAnimation("slide_down");
    ...
    public bool RayCheck(Vector3 dir, bool positive)
    {
        ...
        if (positive)
            ...
            offSetPos = transform.position.y - rayHeightOffset;
        return Physics.Raycast(new Vector3(transform.position.x, offSetPos, transform.position.z), dir, 1f,
            scalableWallsMask);
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawRay(new Vector3(transform.position.x, transform.position.y + rayHeightOffset, transform.position.z),
            Vector3.left);
        Gizmos.DrawRay(new Vector3(transform.position.x, transform.position.y - rayHeightOffset, transform.position.z),
            Vector3.left);
        Gizmos.DrawRay(new Vector3(transform.position.x, transform.position.y - rayHeightOffset, transform.position.z),
